...
(function () {
  ...
  function draw(){
    ...
    ctx.fillStyle=COLOR.text; ctx.font='700 12px "JetBrains Mono", monospace'; ctx.textAlign='left'; ctx.textBaseline='middle'; ctx.fillText('deg', PAD, cy+16);
    for(let j=0;j<N;j++){ const x=gx+j*cell; const val=s.deg[j]; const dead=removed.has(j)&&!layer.has(j); const isLeaf=(val===1);
      rr(x+4,cy+2,cell-8,28,5); ctx.fillStyle=dead?COLOR.gone:(isLeaf?'#fbe7df':'#eef4fa'); ctx.fill(); ctx.lineWidth=1.4; ctx.strokeStyle=dead?COLOR.goneS:(isLeaf?COLOR.leafS:'#a9c4da'); ctx.stroke();
      ctx.fillStyle=dead?COLOR.goneT:COLOR.ink; ctx.font='700 14px "JetBrains Mono", monospace'; ctx.textAlign='center'; ctx.textBaseline='middle'; ctx.fillText(String(val), x+cell/2, cy+16); }
    ctx.fillStyle=COLOR.text; ctx.font='700 12px "JetBrains Mono", monospace'; ctx.textAlign='left'; ctx.textBaseline='middle'; ctx.fillText('n =', gx+N*cell+16, cy+16);
    ctx.fillStyle=s.done?COLOR.centerT:COLOR.coral; ctx.font='700 20px "JetBrains Mono", monospace'; ctx.fillText(String(s.rem), gx+N*cell+56, cy+16);

    // ── BAND 3 · note
    const ty=cy+58, done=!!s.final;
    ctx.fillStyle=COLOR.dim; ctx.font='600 12px "JetBrains Mono", monospace'; ctx.textAlign='left'; ctx.textBaseline='alphabetic';
