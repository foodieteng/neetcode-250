...
(function () {
  ...
  function draw(){
    ...
    for(let j=1;j<=3;j++){ const x=gx+(j-1)*cell; const val=s.parent[j-1]; const isRoot=val<0;
      rr(x+4,cy+2,cell-8,28,5); ctx.fillStyle=isRoot?'#fbe7df':'#eef4fa'; ctx.fill(); ctx.lineWidth=1.4; ctx.strokeStyle=isRoot?COLOR.rootS:'#a9c4da'; ctx.stroke();
    ...
    // current edge
    if(s.cur){ ctx.fillStyle=COLOR.text; ctx.font='700 12px "JetBrains Mono", monospace'; ctx.textAlign='left'; ctx.textBaseline='middle'; ctx.fillText('edge', gx+3*cell+22, cy+16);
      ctx.fillStyle=s.done?COLOR.bad:COLOR.coral; ctx.font='700 16px "JetBrains Mono", monospace'; ctx.fillText('('+s.cur[0]+','+s.cur[1]+')', gx+3*cell+70, cy+16); }
