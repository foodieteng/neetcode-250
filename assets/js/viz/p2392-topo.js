/* ============================================================
   ...
   Walks: k=3, row=[[1,2],[3,2]], col=[[2,1],[3,2]]
     rowOrder=[1,3,2]  colOrder=[3,2,1]  matrix=[[0,0,1],[3,0,0],[0,2,0]]
...
(function () {
  ...
  const K = 3;
  // node layouts (local coords inside each band's DAG area)
  const rowNodes = { 1:{x:36,y:12}, 3:{x:150,y:12}, 2:{x:93,y:74} };
  const rowEdges = [[1,2],[3,2]];
  const colNodes = { 3:{x:30,y:44}, 2:{x:110,y:44}, 1:{x:190,y:44} };
  const colEdges = [[2,1],[3,2]];

  // ── build Kahn steps generically ──
  ...
  const R = kahn(rowNodes, rowEdges);   // order [1,3,2]
  const C = kahn(colNodes, colEdges);   // order [3,2,1]
  const rowIdx = {}; R.order.forEach((v,i)=>rowIdx[v]=i);
  const colIdx = {}; C.order.forEach((v,i)=>colIdx[v]=i);
  const placeSeq = [1,2,3];             // place in value order (as the code does)
  ...
  // row topo steps (skip the initial snapshot index 0 as its own step, fold into pops)
  ...
  snap({ phase:'done', rSnap:R.snaps.length-1, cSnap:C.snaps.length-1, placed:placeSeq.length,
    text:'<strong>完成</strong>：其餘格子填 0。若任一 DAG 有<strong>環</strong>（拓樸長度 ≠ k），無解 → 回傳<strong>空矩陣</strong>。' });
  ...
  function arrow(x1,y1,x2,y2,r1,r2){
    const dx=x2-x1, dy=y2-y1, L=Math.hypot(dx,dy); const ux=dx/L, uy=dy/L;
    ...
    ctx.fillStyle=COLOR.edge; ctx.beginPath();
    ctx.moveTo(ex,ey);
  ...
  function drawDAG(originX, originY, nodes, edges, snapObj, order, idxMap, kind, active){
    const nodeR=19;
    // edges
    ...
    for (const n of Object.keys(nodes)){
      const nd=nodes[n]; const cx=originX+nd.x, cy=originY+nd.y;
      ...
      // indegree badge (only while pending)
      if(!inOrder){
        const d=snapObj.indeg[n];
        ctx.fillStyle=COLOR.coral; ctx.font='700 9px "JetBrains Mono", monospace';
        ctx.textBaseline='bottom'; ctx.textAlign='center';
        ctx.fillText('in:'+d, cx, cy-nodeR-2);
      }
    }
    // order chips to the right
  ...
  function bandNote(w, y, line1, line2, hot){
    const PAD=26, boxW=280, x=w-PAD-boxW;
    const chipsEnd = PAD+8+240+K*(40+10);   // right edge of order chips
    if (x < chipsEnd + 16) return;          // no room on narrow canvases

    rr(x, y, boxW, 52, 4);
    ctx.fillStyle = hot ? '#fbeee8' : '#fafaf6';
    ctx.fill();
    ...
    ctx.fillText(line1, x+14, y+18);
    ctx.fillStyle = COLOR.ink;
    ctx.fillText(line2, x+14, y+37);
  }
  ...
  function draw(){
    ...
    ctx.fillText('BAND 1 · rowConditions DAG → 拓樸 → 列索引', PAD, 26);
    drawDAG(PAD+8, 74, rowNodes, rowEdges, rSnap, rOrder, rowIdx, 'row', s.phase==='row');
    bandNote(w, 92, 'rowOrder 的第 i 個數字', '→ 放到矩陣的第 i 「列」', s.phase==='row');

    // BAND 2 · col DAG
    ctx.fillStyle= s.phase==='col'?COLOR.coral:COLOR.dim;
    ctx.font='600 12px "JetBrains Mono", monospace'; ctx.textAlign='left'; ctx.textBaseline='alphabetic';
    ctx.fillText('BAND 2 · colConditions DAG → 拓樸 → 行索引', PAD, 192);
    drawDAG(PAD+8, 232, colNodes, colEdges, cSnap, cOrder, colIdx, 'col', s.phase==='col');
    bandNote(w, 258, 'colOrder 的第 i 個數字', '→ 放到矩陣的第 i 「行」', s.phase==='col');

    // BAND 3 · grid placement
    ctx.fillStyle= (s.phase==='place'||s.phase==='done')?COLOR.coral:COLOR.dim;
    ctx.font='600 12px "JetBrains Mono", monospace'; ctx.textAlign='left'; ctx.textBaseline='alphabetic';
    ctx.fillText('BAND 3 · 放到 (row, col) 交叉格', PAD, 336);

    const cell=44, gx=PAD+40, gy=352;
    // column headers c0..c2, row headers r0..r2
    ...
    // which values are placed so far
    ...
    const px=gx+K*cell+40, py=352;
    ctx.fillStyle=COLOR.dim; ctx.font='600 11px "JetBrains Mono", monospace';
    ctx.textAlign='left'; ctx.textBaseline='top';
    ctx.fillText('pos[v] = (row, col)', px, py-2);
    for(let i=0;i<placeSeq.length;i++){
      const v=placeSeq[i]; const y=py+18+i*26;
      const shown = i<s.placed;
      ctx.fillStyle = shown?COLOR.text:COLOR.grid;
